...
def main():
    ...
    parser.add_argument('--fast',       action='store_true',
                        help='Fast mode: skip full grid search, use RF importance only')
    args = parser.parse_args()

    set_seed(config.RANDOM_SEED)
    outcome_col = config.get_outcome_column(args.task)

    # ── Load and min-max normalise ────────────────────────────────────────────
    data_df = load_data()
    ...
    if args.train:
        ...
        candidates = mrmr_select_candidates(
            X_train, y_train, n_candidates=20
        )

        if args.fast:
            # Fast mode: skip exhaustive grid, use first N features + intermediate
            ...
            # ── Full Appendix S1 grid search ──────────────────────────────────
            print(f"\n=== Step 2: Grid search (all feature combos × 3 samplings) ===")
    ...
    if args.evaluate:
        ...
        # Predict — threshold 0.5 for sensitivity/specificity (Appendix S1)
        y_prob = model.predict_proba(X_test_sel.values)[:, 1]
        y_pred = (y_prob >= 0.5).astype(int)

        test_m = calculate_metrics(y_test, y_pred, y_prob, threshold=0.5)
        print_metrics(test_m, f"Test Set ({args.task})")
        print("\nNote: Sensitivity and specificity use threshold=0.5 per Appendix S1.")
        print("AUC is the primary metric (threshold-invariant).")
