...
def run_graph_building(task='LR'):
    ...
    from graph_builder import GraphBuilder, fit_dataset_scaler
    ...
    # ── Z-score scaler (Joseph's ScalerGeneration.py approach) ───────────────
    # Fit on training patients only, then pass to GraphBuilder.
    # If no split exists yet, fall back to per-patient GTV-relative norm.
    scaler     = None
    split_dir  = config.OUTPUT_DIR / 'splits'
    train_file = split_dir / f'train_ids_{task}.npy'

    if scaler_path.exists():
        # Load previously fitted scaler (e.g., on a re-run)
        import joblib
        ...
        print("  then re-run graph stage to use z-score normalisation.")
